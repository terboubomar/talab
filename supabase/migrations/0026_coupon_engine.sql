...
-- Explicit coupon codes are supported end-to-end in this migration. auto_apply is
-- stored in the rule model and will be activated separately after explicit-code flow
...
create index if not exists coupons_tenant_status_idx on public.coupons(tenant_id, status, starts_at, ends_at);
...
create index if not exists coupon_scopes_coupon_idx on public.coupon_scopes(coupon_id, scope_type);
...
create unique index if not exists coupon_reservations_active_cart_uidx
  on public.coupon_reservations(cart_id)
  where status in ('reserved','attached');
create index if not exists coupon_reservations_coupon_status_idx
  on public.coupon_reservations(coupon_id, status, expires_at);
create index if not exists coupon_reservations_order_idx on public.coupon_reservations(order_id);
...
alter table public.orders
  ...
  add column if not exists coupon_reservation_id uuid references public.coupon_reservations(id) on delete set null;
...
-- Coupon administration is direct staff-side CRUD under existing RBAC.
...
-- Weekdays use PostgreSQL DOW: Sunday=0 ... Saturday=6. Cross-midnight windows work.
...
begin
  ...
  loop
    if v_window ? 'weekdays' and not exists (
      select 1 from jsonb_array_elements_text(v_window->'weekdays') d where d::int = v_dow
    ) then continue; end if;
    v_start := coalesce(nullif(v_window->>'start','')::time, '00:00'::time);
    v_end := coalesce(nullif(v_window->>'end','')::time, '23:59:59'::time);
    if (v_start <= v_end and v_time between v_start and v_end)
       or (v_start > v_end and (v_time >= v_start or v_time <= v_end)) then
      return true;
    end if;
  end loop;
  return false;
...
create or replace function app.coupon_scope_allows(p_coupon_id uuid, p_order_id uuid, p_source text default 'web')
returns boolean
language plpgsql
stable
set search_path = public, app
...
declare
  ...
  v_group_id uuid;
begin
  select o.*, c.customer_group_id into v_order
  from public.orders o join public.customers c on c.id=o.customer_id
  where o.id=p_order_id;
  if v_order.id is null then return false; end if;

  if exists(select 1 from public.coupon_scopes where coupon_id=p_coupon_id and scope_type='order_type')
     and not exists(select 1 from public.coupon_scopes where coupon_id=p_coupon_id and scope_type='order_type' and scope_id=v_order.order_type::text) then return false; end if;
  if exists(select 1 from public.coupon_scopes where coupon_id=p_coupon_id and scope_type='source')
     and not exists(select 1 from public.coupon_scopes where coupon_id=p_coupon_id and scope_type='source' and lower(scope_id)=lower(coalesce(p_source,'web'))) then return false; end if;
  if exists(select 1 from public.coupon_scopes where coupon_id=p_coupon_id and scope_type='branch')
     and not exists(select 1 from public.coupon_scopes where coupon_id=p_coupon_id and scope_type='branch' and scope_id=v_order.branch_id::text) then return false; end if;
  if exists(select 1 from public.coupon_scopes where coupon_id=p_coupon_id and scope_type='customer')
     and not exists(select 1 from public.coupon_scopes where coupon_id=p_coupon_id and scope_type='customer' and scope_id=v_order.customer_id::text) then return false; end if;
  if exists(select 1 from public.coupon_scopes where coupon_id=p_coupon_id and scope_type='customer_group')
     and not exists(select 1 from public.coupon_scopes where coupon_id=p_coupon_id and scope_type='customer_group' and scope_id=coalesce(v_order.customer_group_id::text,'')) then return false; end if;
  return true;
end;
$$;
...
create or replace function app.coupon_discount_amount(p_coupon_id uuid, p_eligible_subtotal numeric)
returns numeric
language sql
stable
set search_path = public
as $$
  select round(greatest(least(
    case when c.discount_type='percent' then p_eligible_subtotal*c.value/100 else c.value end,
    coalesce(c.max_discount, 999999999::numeric),
    p_eligible_subtotal
  ),0),2)
  from public.coupons c where c.id=p_coupon_id;
$$;

-- Public quote + atomic reservation. Prices are recomputed from live catalog data.
...
as $$
declare
  v_branch record; v_coupon public.coupons%rowtype; v_customer record;
  v_item jsonb; v_product record; v_mod jsonb; v_mod_id uuid; v_mod record;
  v_qty int; v_price numeric; v_line numeric; v_mod_sum numeric;
  v_subtotal numeric:=0; v_eligible numeric:=0; v_deposit numeric:=0; v_delivery numeric:=0;
  v_discount numeric:=0; v_delivery_discount numeric:=0; v_total numeric:=0;
  v_reservation_id uuid; v_expires timestamptz:=now()+interval '10 minutes';
  v_used int; v_customer_used int; v_has_item_scope boolean;
  v_zone record;
begin
  ...
  select * into v_coupon from public.coupons c
   where c.tenant_id=v_branch.tenant_id and lower(c.code)=lower(trim(p_code))
   for update;
  ...
  if exists(select 1 from public.coupon_scopes where coupon_id=v_coupon.id and scope_type='order_type')
     and not exists(select 1 from public.coupon_scopes where coupon_id=v_coupon.id and scope_type='order_type' and scope_id=p_order_type::text) then raise exception 'coupon_scope_mismatch'; end if;
  if exists(select 1 from public.coupon_scopes where coupon_id=v_coupon.id and scope_type='source')
     and not exists(select 1 from public.coupon_scopes where coupon_id=v_coupon.id and scope_type='source' and lower(scope_id)=lower(coalesce(p_source,'web'))) then raise exception 'coupon_scope_mismatch'; end if;
  if exists(select 1 from public.coupon_scopes where coupon_id=v_coupon.id and scope_type='branch')
     and not exists(select 1 from public.coupon_scopes where coupon_id=v_coupon.id and scope_type='branch' and scope_id=p_branch_id::text) then raise exception 'coupon_scope_mismatch'; end if;
  if exists(select 1 from public.coupon_scopes where coupon_id=v_coupon.id and scope_type='customer')
     and (v_customer.id is null or not exists(select 1 from public.coupon_scopes where coupon_id=v_coupon.id and scope_type='customer' and scope_id=v_customer.id::text)) then raise exception 'coupon_scope_mismatch'; end if;
  if exists(select 1 from public.coupon_scopes where coupon_id=v_coupon.id and scope_type='customer_group')
     and (v_customer.customer_group_id is null or not exists(select 1 from public.coupon_scopes where coupon_id=v_coupon.id and scope_type='customer_group' and scope_id=v_customer.customer_group_id::text)) then raise exception 'coupon_scope_mismatch'; end if;
  ...
  for v_item in select * from jsonb_array_elements(p_items) loop
    ...
    v_mod_sum:=0;
    for v_mod in select * from jsonb_array_elements(coalesce(v_item->'modifier_ids','[]'::jsonb)) loop
      v_mod_id:=(v_mod#>>'{}')::uuid;
      select m.id,m.price into v_mod from public.modifiers m
      ...
      if v_mod.id is null then raise exception 'invalid_modifier'; end if;
      v_mod_sum:=v_mod_sum+(v_mod.price*v_qty);
    end loop;
    ...
    if not v_has_item_scope
       ...
       or exists(select 1 from public.coupon_scopes where coupon_id=v_coupon.id and scope_type='category' and scope_id=v_product.category_id::text) then
  ...
  if p_order_type='delivery' then
    ...
    select dz.fee,dz.min_order,dz.below_min_fee into v_zone from public.delivery_zones dz
  ...
  if v_coupon.per_customer_limit is not null then
    select count(*) into v_customer_used from public.coupon_redemptions cr
      join public.orders o on o.id=cr.order_id
      where cr.coupon_id=v_coupon.id and cr.status='redeemed'
        and ((v_customer.id is not null and cr.customer_id=v_customer.id) or (v_customer.id is null and false));
  ...
  on conflict (cart_id) where status in ('reserved','attached') do update
    set coupon_id=excluded.coupon_id,customer_id=excluded.customer_id,customer_phone=excluded.customer_phone,
  ...
  return jsonb_build_object(
    ...
    'total',v_total,'expires_at',v_expires,'success_msg_ar',v_coupon.success_msg_ar,
    'success_msg_en',v_coupon.success_msg_en
  );
end;
$$;
...
-- Coupon-aware checkout wrapper. Existing v2 remains stable for compatibility.
...
begin
  select r.order_id,r.total into v_order_id,v_total from public.storefront_place_order_v2(
    p_tenant_slug,p_branch_id,p_order_type,p_customer_name,p_customer_phone,p_notes,p_items,
    p_area_id,p_lat,p_lng,p_address_text,p_payment_method
  ) r;
  ...
  if v_coupon.ends_at is not null and now()>v_coupon.ends_at then raise exception 'coupon_expired'; end if;
  if not app.coupon_daypart_allows(v_coupon.day_parting_json,now()) then raise exception 'coupon_outside_schedule'; end if;
  if not app.coupon_scope_allows(v_coupon.id,v_order.id,p_source) then raise exception 'coupon_scope_mismatch'; end if;
  ...
  update public.orders set coupon_id=v_coupon.id,coupon_code=v_coupon.code,coupon_reservation_id=v_res.id,
    ...
    where id=v_order.id;
  update public.coupon_reservations set order_id=v_order.id,amount=v_discount,
...
-- Keep coupon usage consistent with online payment and cancellation outcomes.
...
begin
  ...
  if new.payment_method='online' and new.payment_status in ('paid','partially_refunded','refunded')
     and old.payment_status is distinct from new.payment_status then
    perform app.coupon_finalize_order(new.id);
  elsif new.payment_method='online' and new.payment_status in ('failed','cancelled')
     and old.payment_status is distinct from new.payment_status then
    update public.coupon_reservations set status='released',updated_at=now()
      where id=new.coupon_reservation_id and status='attached';
  end if;
  if new.status='cancelled' and old.status is distinct from new.status then
    update public.coupon_redemptions set status='voided',voided_at=now()
      where order_id=new.id and status='redeemed';
    update public.coupon_reservations set status='released',updated_at=now()
      where order_id=new.id and status in ('reserved','attached');
  end if;
  return new;
end;
$$;
