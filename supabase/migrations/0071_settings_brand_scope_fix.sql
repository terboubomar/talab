-- 0071_settings_brand_scope_fix.sql
-- brand_scope is generated from brand_id. Tenant-wide settings use brand_id IS NULL.
...
begin
  ...
  if p_group_key not in ('general','order','checkout','payment','dinein','application','website','pages','text-control','delivery-areas','working-times','notifications','links-page','activity-log','other') then raise exception 'invalid_settings_group'; end if;

  select * into v_tenant from public.tenants where id = v_tenant_id;

  if p_group_key = 'general' then
    v_default := jsonb_build_object(
      'languages', to_jsonb(v_tenant.langs), 'default_language', trim(v_tenant.default_lang),
      'vat_number', coalesce(v_tenant.vat_number, ''), 'tax_inclusive', v_tenant.tax_inclusive,
      'tax_certificate_path', null, 'customer_addresses_enabled', true,
    ...
    v_default := jsonb_build_object(
      ...
      'add_to_cart_before_order_type', true, 'show_driver_info', true,
      'pickup_confirmation_message', true, 'customer_cancel_while_waiting', false,
      'show_branch_phone', true, 'show_delivery_time_in_cart', true,
      'send_details_whatsapp', false, 'handed_to_driver_button', true,
      'location_time_before_products', false, 'product_suggestions', true,
      'location_method', 'map', 'order_expiry_minutes', 30
    );
  elsif p_group_key = 'checkout' then
    v_default := jsonb_build_object(
      'show_customer_info', true, 'show_delivery_info', true, 'show_pickup_info', true,
      'show_curbside_info', true, 'show_time', true, 'show_coupon_codes', true,
      'show_order_details', true, 'show_price_details', true, 'show_product_notes', true,
      'show_order_notes', true, 'order_notes_required', false, 'allow_quantity_edit', true,
      'show_price_while_adding', true, 'show_preparation_prompt', true,
    ...
    v_default := jsonb_build_object(
      'cash_enabled', true, 'pos_device_enabled', false, 'stc_pay_barcode_enabled', false,
      'wallet_with_cash_enabled', false, 'tamara_banners_enabled', false,
      'wallet_max_per_order', null, 'online_payment_name_ar', '',
      'online_payment_name_en', '', 'online_payment_logo_path', null
    );
  end if;

  select ts.value into v_saved
  from public.tenant_settings ts
  where ts.tenant_id = v_tenant_id and ts.brand_id is null and ts.group_key = p_group_key;
...
begin
  ...
  if p_group_key = 'general' then
    select coalesce(array_agg(value), array[]::text[]) into v_langs
    from jsonb_array_elements_text(coalesce(p_value->'languages','[]'::jsonb));
    if cardinality(v_langs) = 0 or exists(select 1 from unnest(v_langs) x where x not in ('ar','en')) then raise exception 'invalid_languages'; end if;
    ...
    update public.tenants set langs = v_langs, default_lang = v_default_lang,
      vat_number = nullif(trim(coalesce(p_value->>'vat_number','')), ''),
      tax_inclusive = coalesce((p_value->>'tax_inclusive')::boolean, true), updated_at = now()
    where id = v_tenant_id;
  end if;

  insert into public.tenant_settings(tenant_id, brand_id, group_key, value, updated_at, updated_by)
  values(v_tenant_id, null, p_group_key, p_value, now(), v_staff_id)
