      ...
      <div className="card-surface w-full max-w-sm p-8 text-center">
        ...
        {orderId ? (
          <p className="mt-3 text-xs text-muted-foreground">
            رقم الطلب <span dir="ltr" className="font-bold text-foreground">#{orderId.slice(0, 8)}</span>
          </p>
        ) : null}
        {state !== "checking" ? (
          <div className="mt-6 grid gap-2">
            {state !== "paid" ? (
              <Link to="/checkout" className="rounded-pill border border-border px-5 py-3 text-sm font-bold">
                العودة للدفع
              </Link>
            ) : null}
            <Link to="/" className="rounded-pill bg-brand px-5 py-3 text-sm font-bold text-brand-ink">
              العودة للرئيسية
            </Link>
          </div>
        ) : null}
