...
import { BadgePercent, ImageOff, MapPin, ShoppingBag } from "lucide-react";
...
import { fetchMemberPriceFlags } from "@/lib/member-pricing";
...
function MenuPage() {
  ...
  const { data: memberPriceIds = [] } = useQuery({
    queryKey: ["storefront_member_price_flags"],
    queryFn: fetchMemberPriceFlags,
    retry: false,
  });
  ...
  const memberPriceSet = useMemo(() => new Set(memberPriceIds), [memberPriceIds]);
        ...
        >
          ...
          {selection ? (
            <span className="min-w-0 flex-1">
              <span className="block text-[11px] leading-none text-ink-3">
      ...
      <div className="space-y-3">
        {products.map((product) => (
          <ProductRow
            key={product.id}
            product={product}
            hasMemberPrice={memberPriceSet.has(product.id)}
            onOpen={onOpen}
          />
        ))}
      </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between gap-2 p-1">
        <div className="flex-grow">
          <div className="flex flex-wrap items-center gap-2">
            ...
            {hasMemberPrice ? (
              <span className="inline-flex items-center gap-1 rounded-pill bg-brand-soft px-2 py-0.5 text-[10px] font-medium text-brand">
                <BadgePercent aria-hidden className="size-3" />
                سعر أعضاء متوفر
              </span>
            ) : null}
            {outOfStock ? (
              <span className="rounded-pill bg-danger/10 px-2 py-0.5 text-[10px] font-medium text-danger">غير متوفر</span>
            ) : null}
          </div>
          {product.desc_ar ? (
            <p className="mt-1 line-clamp-2 text-sm leading-[1.6] text-ink-2">{product.desc_ar}</p>
          ) : null}
        </div>
        <div className="flex flex-wrap items-center justify-between gap-2">
          <span className="text-sm font-semibold text-ink tabular-nums">
            {formatSAR(Number(product.price ?? 0))}
          </span>
          {product.calories != null ? (
            <span className="text-xs text-ink-3">{formatCalories(product.calories)}</span>
          ) : null}
        </div>
      </div>
  ...
  return (
    <div className="rounded-card border border-line bg-surface-raised p-4">
      ...
      <div className="mt-3 divide-y divide-line-soft">
          ...
          <div key={line.key} className="py-3 text-xs">
            <div className="flex justify-between gap-2">
              ...
              <span className="shrink-0 font-semibold text-ink tabular-nums">
                {formatSAR(line.unitPrice * line.quantity)}
              </span>
            </div>
        ...
        {cart.length > 6 ? (
          <p className="py-2 text-xs text-ink-3">+ {cart.length - 6} أصناف أخرى</p>
        ) : null}
      </div>
      <div className="mt-3 flex justify-between border-t border-line pt-3 text-sm font-semibold">
        <span>الإجمالي</span>
        <span className="tabular-nums">{formatSAR(total)}</span>
      </div>
      <button
        type="button"
        onClick={onCheckout}
        className="mt-3 min-h-11 w-full rounded-card bg-brand px-4 py-3 text-sm font-semibold text-brand-ink"
      >
        تنفيذ الطلب
      </button>
    </div>
