...
const NAV_ITEMS: {
  ...
  { label: "المنتجات", to: "/admin/products", perm: "menus.view", enabled: true },
  {
    label: "الفروع",
    to: "/admin/branches",
    anyPerm: ["branches.view", "branches.busy.toggle"],
    enabled: true,
  },
  {
    label: "أدوات التسويق",
    to: "/admin/marketing-tools",
    anyPerm: ["marketing.tools", "coupons.view", "marketing.loyalty", "marketing.cashback"],
    enabled: true,
  },
  ...
  { label: "الموظفين", to: "/admin/staff", perm: "staff.view", enabled: true },
  {
    label: "المدفوعات",
    to: "/admin/payments",
    anyPerm: ["payments.view", "orders.refund"],
    enabled: true,
  },
  { label: "متجر التطبيقات", to: "/admin/apps", perm: "integrations.manage", enabled: true },
  { label: "الإعدادات", to: "/admin/settings", perm: "settings.manage", enabled: true },
];
          ...
          {visibleItems.map((item) => {
            ...
            return (
              <Link
                key={item.to}
                to={item.to}
                className={`flex items-center rounded-card px-3 py-2.5 text-sm font-bold ${active ? "bg-brand text-brand-ink" : "text-foreground hover:bg-secondary"}`}
              >
                {item.label}
              </Link>
            );
